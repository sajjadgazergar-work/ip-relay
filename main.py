# oc-rotator — OpenCode free-tier IP rotator relay
#
# Sits between 9router (VPS) and opencode.ai/zen/v1. Each unique egress IP
# carries its own free-tier quota, so this relay rotates through a proxy
# pool on 429 (FreeUsageLimitError) and lets one baseURL serve "unlimited"
# daily free-model traffic. The VPS IP never touches opencode.
#
# VERIFIED (2026-08-07): free tier needs `Authorization: Bearer public`
# (9router ships the same). Fresh IP + Bearer public -> 200.
# Fresh IP + fake key -> AuthError (key is validated). Burned IP + any
# key -> FreeUsageLimitError. So: rotate IPs, send Bearer public.
#
# Deploy target: Railway. Env:
#   OPENCODE_API_KEY   override upstream key (default: "public")
...
#   PORT               Railway injects this (default 8080)
...
log = logging.getLogger("oc-rotator")
logging.basicConfig(level=logging.INFO, format="%(asctime)s %(levelname)s %(message)s")

OPENCODE_API_KEY = os.environ.get("OPENCODE_API_KEY", "public").strip()
OPENCODE_BASE_URL = os.environ.get("OPENCODE_BASE_URL", "https://opencode.ai/zen/v1").rstrip("/")
...
app = FastAPI(title="oc-rotator")

# ── proxy pool state ──────────────────────────────────────────────
pool = {"proxies": [], "updated": 0.0, "lock": asyncio.Lock()}
# proxy -> cooldown_until (burned IPs get parked for a while)
cooldowns: dict[str, float] = {}
# set when the direct (Railway) egress itself hits FreeUsageLimit — on Railway
# it's a fresh IP, on the VPS it's burned; this flag makes the behavior correct
# in both places
        ...
        for url in PROXY_LIST_URLS:
            try:
                ...
                for line in r.text.splitlines():
                    ...
                    if not line:
                        continue
                    # formats: ip:port, ip:port:user:pass, or ip:port@user:pass
                    if "://" in line:
                        continue
                    parts = line.split(":")
                    if len(parts) >= 2:
...
async def proxy_works(proxy: str) -> bool:
    """Real test: can we complete a 1-token opencode free request through it?
    This simultaneously proves (a) the proxy is alive and (b) its IP is not
    burned for the free tier — exactly what the relay needs."""
    try:
        async with httpx.AsyncClient(timeout=12, proxy=f"http://{proxy}") as client:
            ...
            if r.status_code == 429:
                # this proxy's IP is already burned — park it longer
...
async def refresh_pool(force: bool = False) -> None:
    async with pool["lock"]:
        ...
        log.info("fetched %d proxy candidates (capped %d)", len(candidates), PROXY_MAX_CANDIDATES)
...
async def get_egress_candidates() -> list[tuple[str | None, str]]:
    """Return [(proxy_or_None, label)] — direct (Railway) lane first, then pool."""
    global direct_burned_until
    async with pool["lock"]:
        alive = [p for p in pool["proxies"] if cooldowns.get(p, 0) <= time.time()]
    lanes: list[tuple[str | None, str]] = []
    if time.time() >= direct_burned_until:
        lanes.append(("direct", "railway-direct"))
    # shuffle so direct isn't always hammered first; keep direct near the front
...
def mark_burn(proxy: str | None, label: str, how_long: float = 3600.0) -> None:
    ...
    elif label == "railway-direct":
        # direct egress burned (VPS local test, or Railway IP exhausted) — park 30 min
...
def build_upstream_headers(client_auth: str | None, request: Request) -> dict:
    ...
    key = OPENCODE_API_KEY or (client_auth or "")
    if key:
...
def is_free_usage_429(body: bytes, status: int) -> bool:
    if status != 429:
        return False
    try:
        data = json.loads(body)
        err = data.get("error", {})
        return err.get("type") == "FreeUsageLimitError" or "Rate limit" in str(err.get("message", ""))
    except Exception:
        return False
...
async def safe_aiter(ait):
    ...
    try:
        ...
        # upstream hiccup mid-stream — emit a final SSE [DONE] so clients
        # don't hang waiting for the terminator
        try:
            yield b"data: [DONE]\n\n"
        except Exception:
            pass
...
async def relay(payload: dict, path: str, stream: bool, headers: dict, timeout: float, method: str = "POST"):
    ...
    if not lanes:
        # no lane at all (direct proven burned + empty pool) — fail fast
    ...
    for proxy, label in lanes:
        try:
            ...
            if proxy:
                ...
                # direct (railway) lane: never let it hang the whole request
                client_kwargs["timeout"] = httpx.Timeout(min(timeout, 20), connect=10)
            async with httpx.AsyncClient(**client_kwargs) as client:
                url = f"{OPENCODE_BASE_URL}/{path}"
                if stream:
                    async with client.stream(method, url, json=payload if method == "POST" else None, headers=headers) as resp:
                        if resp.status_code == 429:
                            body = await resp.aread()
                            if is_free_usage_429(body, 429):
                                mark_burn(proxy, label)
                                last_err = (429, body)
                                continue
                        return resp.status_code, dict(resp.headers), resp.aiter_raw()
                else:
                    resp = await client.request(method, url, json=payload if method == "POST" else None, headers=headers)
                    body = resp.content
                    if resp.status_code == 429 and is_free_usage_429(body, 429):
                        mark_burn(proxy, label)
                        last_err = (429, body)
                        continue
...
@app.get("/healthz")
async def healthz():
    return {"ok": True, "pool": len([p for p in pool["proxies"] if cooldowns.get(p, 0) <= time.time()]), "updated": pool["updated"], "key": mask_key(OPENCODE_API_KEY)}
